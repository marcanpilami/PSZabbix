function New-ZbxUser 
{
    ...
    begin
    {
        ...
        {
            $media += @{
                mediatypeid = @(Get-ZbxMediaType -Type email)[0].mediatypeid
                sendto = $MailAddress
    ...
    end
    {
        ...
        $id = Invoke-ZbxZabbixApi $session "user.create"  $prms
        Get-ZbxUser -Session $Session -Id $id.userids
    }
}
