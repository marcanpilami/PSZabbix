...
function Get-ZbxApiVersion
{
    ...
    .PARAMETER Session
    A session hashtable created by calling New-ZbxApiSession. If not specified, the function will look for $script:latestSession
    ...
    .EXAMPLE
    Get-ZbxApiVersion $(New-ZbxApiSession "http://myserver/zabbix/api_jsonrpc.php" (Get-Credentials MyAdminLogin))
    Gets the API version of the server while setting up a new API session. 

    .EXAMPLE
    Get-ZbxApiVersion http://myZabbix.myNet.org/zabbix/api_jsonrpc.php
    ...
    $requestBody = New-ZbxJsonrpcRequest "apiinfo.version" @{}
    $httpResult = ""
    try 
    {
